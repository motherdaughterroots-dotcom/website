      ...
      <nav className="max-w-7xl mx-auto px-5 sm:px-8 flex items-center justify-between h-[72px] sm:h-[82px]">
        ...
        <Link to="/" onClick={() => setOpen(false)}
          ...
          <div className="hidden sm:flex flex-col leading-tight">
            <span className="font-display text-[16px] font-medium text-green">Mother Daughter Roots</span>
            ...
            <NavLink key={l.to} to={l.to} end={l.to === '/'}
              ...
              {({ isActive }) => (
                <span className="relative py-1">
                  {l.label}
                  {/* Courses gets a subtle accent dot to make it pop */}
                  {l.to === '/courses' && !isActive && (
                    <span className="absolute -top-0.5 -right-2 w-1 h-1 rounded-full bg-[var(--color-terracotta)]" />
                  )}
                  {isActive && (
            ...
            <div className="flex flex-col px-6 py-5 gap-0.5">
              {links.map(l => (
                <NavLink key={l.to} to={l.to} end={l.to === '/'} onClick={() => setOpen(false)}
                  className={({ isActive }) =>
                    `py-3.5 text-[18px] font-display border-b border-[var(--color-cream-line)]/50 flex items-center justify-between
                  ...
                  {({ isActive }) => (
                    <>
                      {l.label}
                      {l.to === '/courses' && !isActive && (
                        <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-[var(--color-terracotta)]/10 text-[var(--color-terracotta)]">
                          New
                        </span>
                      )}
                    </>
