...
export default function ProductCard({ product, index = 0 }) {
  ...
  // Display-only anchor price — purely visual, never used in cart/checkout math.
  const anchorPrice = product.price + 50;
      ...
      <Link to={`/product/${product.id}`} className="block">
        ...
        <div className="px-1">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-baseline gap-1.5">
              <span className="font-display text-[17px] text-[var(--color-olive)]">₹{product.price}</span>
              <span className="text-[13px] text-[var(--color-bark)]/40 line-through">₹{anchorPrice}</span>
            </div>
            <span className="text-xs text-[var(--color-bark)]/45">{product.netQty}</span>
          </div>
        </div>
