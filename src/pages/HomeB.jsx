// OPTION B — Full-width hero background from Unsplash (herbal/nature imagery)
// Requires internet connection to load the background image
// Hero: dark olive overlay on top of rich herbal flatlay, text in ivory/gold

import { motion } from 'framer-motion';
import { Link } from 'react-router-dom';
import { ArrowRight, Leaf, Sprout, FlaskConical, PackageCheck, Star, Instagram, ShieldCheck, Play } from 'lucide-react';
...
// Beautiful turmeric/herbal flatlay — warm, earthy, perfectly on-brand
// const HERO_BG = 'https://images.unsplash.com/photo-1608571423902-eed4a5ad8108?auto=format&fit=crop&w=1920&q=80';
// const HERO_BG = 'https://images.unsplash.com/photo-1760552070063-b1c5213b5a6f?auto=format&fit=crop&w=1920&q=80';
// const HERO_BG = 'https://images.openai.com/static-rsc-4/un3wue4U1UWOPHypLkE8oA8pGrraq2MlcmS9bhR9C9cj4WkrC0PXdLey9jrmFSjRj-rW0QsjtjwbxPB5DOnzaTI39_OYyCGggLJKKEINS9bFX5JcS-Fq6ezteCEoY3xCeUosNZpto5WXCxCFPa0IugGktKSxFbaNYd3d_V1UtpS53edwIYEaipptY5sgk9NI?purpose=fullsize'
...
const CAT_IMAGES = {
  soaps:      '/images/products/ubtan-soap.jpeg',
  'hair-care':'/images/products/hair-oil.jpeg',
  oils:       '/images/products/coconut-oil.jpeg',
  powders:    '/images/products/beetroot-powder.jpeg',
};
          ...
          {categories.map((cat, i) => (
            <motion.div key={cat.id}
              ...
              <Link to={`/shop?category=${cat.id}`}
                className="group relative flex rounded-2xl overflow-hidden aspect-[3/4]">
                <img src={CAT_IMAGES[cat.id]} alt={cat.name} loading="lazy"
                  className="w-full h-full object-cover object-center transition-transform duration-700 group-hover:scale-110" />
                <div className="absolute inset-0 bg-gradient-to-t from-[var(--color-bark)]/80 via-[var(--color-bark)]/10 to-transparent" />
                <div className="absolute bottom-0 left-0 p-4 sm:p-5">
                  <p className="font-display text-lg sm:text-xl text-white">{cat.name}</p>
                  <span className="text-xs text-white/65 flex items-center gap-1 mt-1 group-hover:gap-2 transition-all duration-300">
